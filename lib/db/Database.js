...
export default class Database {
	...
	createTables(){
		...
		let sql =
			"CREATE TABLE IF NOT EXIST METRIC " +
	...
	getMetrics() {
		...
		db.all(sql, [], (err, rows) => {
			if(err){
				...
				rows.forEach((row) => {
					metrics.add(new Metric(row.id, row.name, Instant.parse(row.startDate),
							Instant.parse(row.endDate), row.activity_type, row.value,
